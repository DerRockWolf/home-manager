...
let
  ...
  enabledContactAccounts = attrValues
    (filterAttrs (_: a: a.thunderbird.enable) config.accounts.contact.accounts);
  ...
  toThunderbirdAddressBook = addressbook:
    let inherit (addressbook) id;
    in {
      "ldap_2.server.addressbook_${id}.description" = addressbook.name;
      # "ldap_2.server.addressbook_${id}.uid" = id; # TODO: check if this is required
      # "ldap_2.server.addressbook_${id}.position" = 0; # TODO: check if this is required
    } // optionalAttrs (addressbook.remote == null) {
      "ldap_2.server.addressbook_${id}.dirType" = 102; # TODO: what is that?
      "ldap_2.server.addressbook_${id}.filename" = "abook-${id}.sqlite";
    }
    // optionalAttrs
    (addressbook.remote != null && addressbook.remote.type == "carddav") {
      "ldap_2.servers.addressbook_${id}.carddav.url" = addressbook.remote.url;
      "ldap_2.servers.addressbook_${id}.carddav.username" =
        addressbook.remote.userName;
    } // optionalAttrs addressbook.thunderbird.readOnly {
      "ldap_2.servers.addressbook_${id}.readOnly" = true;
    };
  # carddav.token
  # .dirType # also exists on carddav addressbooks
  # .filename # also exists on carddav addressbooks
  # maybe add refreshinterval (for calendars) & syncinterval for contacts (0 means disabled but I don't know what the default is)
  ...
  config = mkIf cfg.enable {
    assertions = [
      ...
      (let
        ...
        selectedProfiles = concatMap (a: a.thunderbird.profiles)
          (enabledEmailAccounts ++ enabledCalendarAccounts ++ enabledContactAccounts);
      in {
      ...
      (let
        foundCalendars = filter (a: a.remote.type == "google_calendar")
          enabledCalendarAccounts;
      in {
    ...
    home.file = mkMerge ([{
      ...
      "${thunderbirdProfilesPath}/${name}/user.js" = let
        ...
        addressbooks = f enabledContactAccountsWithId;
      ...
      in {
        text = mkUserJs (builtins.foldl' (a: b: a // b) { } ([
          ...
          ++ (map toThunderbirdAddressBook addressbooks))) profile.extraConfig;
      };
    }));
  };
}
